...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.is_operator(user_id):
        # Operator uchun tugmalar (simplified)
        await query.edit_message_text(f"Operator paneli: {data}")
    elif db.is_targetolog(user_id):
        # Targetolog uchun tugmalar
        if data == "my_leads":
...
async def main():
    ...
    # Admin handlerlarini qo‘shamiz
    for handler in get_handlers():
        application.add_handler(handler)

    # Targetolog handlerlarini qo‘shamiz
    for handler in get_targetolog_handlers():
        application.add_handler(handler)

    # Operator handlerlarini qo‘shamiz
